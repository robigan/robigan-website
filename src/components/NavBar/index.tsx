...
const UniversalWrapper: FC<PropsWithChildren<LinkProps>> = ({ children, ...linkProps }) => {
    const { pathname } = useContext(PathnameContext);

    return (
        <Link {...linkProps}>
            <a className="mx-3 lg:mx-0 my-0 lg:my-3 w-48p h-48p rounded-full" style={pathname === linkProps.href ? { backgroundColor: "#FFB400" } : { backgroundColor: "#3C3C3C" }}>
                {children}
            </a>
        </Link>
    );
